    ...
    <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center p-4 justify-center'>
      <div className='max-w-md w-full'>
        <div className='text-center mb-4 flex items-center justify-between'>
          <button onClick={() => setShowModal(false)} className='text-white p-4 cursor-pointer'>
            <ArrowLeft />
          </button>
        ...
        <div className='flex gap-2 mt-4'>
          <button onClick={()=>{setMode('text'); setMedia(null); setPreviewUrl(null)}} className={`flex flex-1 items-center p-2 gap-2 cursor-pointer justify-center rounded ${mode === "text" ? 'bg-white text-black' : 'bg-zinc-800'}`}>
